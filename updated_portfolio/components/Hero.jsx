/* === Hero: binary rain canvas, portrait, name reveal === */
...
const BinaryRain = () => {
  ...
  React.useEffect(() => {
    ...
    const resize = () => {
      ...
      drops = Array(cols).fill(0).map(() => Math.random() * h / (fontSize * window.devicePixelRatio));
    };
    resize();
    ...
    const draw = () => {
      ...
      for (let i = 0; i < cols; i++) {
        ...
        ctx.fillStyle = bright ? "rgba(255,255,255,0.85)" : `rgba(255,255,255,${0.12 + Math.random() * 0.18})`;
        ctx.fillText(ch, x, y);
...
const Hero = () => {
  ...
  return (
    <section className="hero" id="home">
      ...
      <div className="hero-watermark">उज्वल मोजिद्रा</div>
